"""Mission control's aggregate (operator-experience.md §2): `GET /teams/{id}/pulse` —
the team pulse header numbers plus one overlay row per node."""
...
def test_pulse_idle_org(client, make_org):
    ...
    p = client.get(f"/api/teams/{team['id']}/pulse").json()
    ...
    assert client.get("/api/teams/nope/pulse").status_code == 404
...
def test_pulse_live_overlay(client, make_org, mint_session):
    ...
    p = client.get(f"/api/teams/{team['id']}/pulse?windowMinutes=60").json()
    [node] = p["nodes"]
