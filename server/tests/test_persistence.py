"""SQLite team store — the phase-1 contract on a new backend, plus non-destructive migration."""
...
def test_write_read_roundtrip(tmp_path):
    store = SqliteTeamStore(Db(tmp_path / "canopy.db"))
    store.write(_org("o1"))
...
def test_json_docs_migrate_non_destructively(tmp_path):
    ...
    store = SqliteTeamStore(Db(tmp_path / "canopy.db"), migrate_from=json_dir)
...
def test_migration_does_not_clobber_existing_rows(tmp_path):
    ...
    store2 = SqliteTeamStore(db, migrate_from=json_dir)
