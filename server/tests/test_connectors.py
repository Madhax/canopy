...
def _mk_instance(store, org_id, **kw):
    defaults = dict(
    ...
    defaults.update(kw)
    return store.create(org_id, defaults.pop("pack_key"), defaults.pop("name"), **defaults)


# ------------------------------------------------------------------ resolution
def test_resolution_scope_mask_and_pin(client, org):
    """The §4 precedence vectors: org-wide serves everyone; a node link scopes; a node pin
    outranks org-wide; the mask excludes; disable kills; [] links nobody."""
    store = _connector_store()
    catalog = get_catalog()
    oid = org["id"]
    node = org["agents"][0]["id"]
    other = org["agents"][1]["id"]

    inst = _mk_instance(store, oid)  # org-wide
    ...
    # A node-linked second instance outranks the org-wide one FOR THAT NODE only.
...
def test_readiness_issue_vectors(client, org):
...
# ------------------------------------------------------------------ routes
def test_routes_crud_and_secret_custody(client, org):
    """Secrets go in as plaintext, come back as Secret Store ids, and are never readable."""
    oid = org["id"]
    node = org["agents"][0]["id"]

    packs = client.get(f"/api/organizations/{oid}/connector-packs").json()["packs"]
    ...
    # Re-scope to org-wide via the tri-state linkScope; rotate the secret.
    r = client.put(f"/api/organizations/{oid}/connectors/{inst['id']}", json={
        "linkScope": "org", "secrets": {"scm-token": "ghp_ROTATED"},
    })
    ...
    for body in bad:
        assert client.post(f"/api/organizations/{oid}/connectors", json=body).status_code == 400
    dropped = client.post(f"/api/organizations/{oid}/connectors", json={
        "packKey": "github", "name": "just dropped", "config": {"branchPattern": "canopy/*"},
    })
    assert dropped.status_code == 201
    v = client.post(
        f"/api/organizations/{oid}/connectors/{dropped.json()['id']}/verify"
    ).json()
    assert v["ok"] is False
    assert any(c["name"] == "config:owner" and not c["ok"] for c in v["checks"])

    r = client.delete(f"/api/organizations/{oid}/connectors/{inst['id']}")
    assert r.status_code == 204
    remaining = client.get(f"/api/organizations/{oid}/connectors").json()["instances"]
    assert [i["name"] for i in remaining] == ["just dropped"]


def test_local_git_instance_binds_the_repo_source(client, org, tmp_path):
    ...
    iid = r.json()["id"]
    v = client.post(f"/api/organizations/{oid}/connectors/{iid}/verify").json()
    assert v["ok"] is True
    client.put(f"/api/organizations/{oid}/connectors/{iid}",
               json={"config": {"source": str(tmp_path / "nowhere")}})
    v = client.post(f"/api/organizations/{oid}/connectors/{iid}/verify").json()
    assert v["ok"] is False


def test_push_branch_round_trips_to_the_source(client, org, tmp_path):
    """The pr-create executor's first half: a canopy/* branch lands on the org's source
    ...
    client.post(f"/api/organizations/{oid}/connectors", json={
