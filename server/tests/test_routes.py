...
def test_save_persists_with_errors_and_bumps_updatedat(client, make_org):
    ...
    team["agents"][0]["salary"]["perAssignmentAllowance"] = 0  # introduce SALARY_INVALID
    r = client.put(f"/api/teams/{team['id']}", json=team)
    assert r.status_code == 200
    body = r.json()
    assert any(i["code"] == "SALARY_INVALID" for i in body["issues"])
    assert body["document"]["updatedAt"] != team["updatedAt"]
    # persisted despite the error
    again = client.get(f"/api/teams/{team['id']}").json()
    assert again["agents"][0]["salary"]["perAssignmentAllowance"] == 0


def test_stale_write_conflict(client, make_org):
    team = make_org(seed={"kind": "root", "roleKey": "engineering-lead"})
    first = client.put(f"/api/teams/{team['id']}", json=team)
    assert first.status_code == 200
    # team still carries the OLD updatedAt -> conflict
    second = client.put(f"/api/teams/{team['id']}", json=team)
...
def test_immutable_fields_reimposed(client, make_org):
    team = make_org(seed={"kind": "root", "roleKey": "engineering-lead"})
    original_created = team["createdAt"]
    team["createdAt"] = "1999-01-01T00:00:00Z"  # attempt to change immutable field
    r = client.put(f"/api/teams/{team['id']}", json=team)
    assert r.status_code == 200
...
def test_id_mismatch_rejected(client, make_org):
    team = make_org(seed={"kind": "root", "roleKey": "engineering-lead"})
    team["id"] = "different"
    r = client.put("/api/teams/different", json=team)
...
def test_validate_modes(client, make_org):
    team = make_org(seed={"kind": "blank"})
    draft = client.post(f"/api/teams/{team['id']}/validate?mode=draft").json()["issues"]
    export = client.post(f"/api/teams/{team['id']}/validate?mode=export").json()["issues"]
...
def test_export_gates_on_errors(client, make_org):
    ...
    r = client.get(f"/api/teams/{blank['id']}/export")
...
def test_export_success_is_canonical(client, make_org):
    team = make_org(seed={"kind": "formation", "formationKey": "product-engineering-pod"})
    r = client.get(f"/api/teams/{team['id']}/export")
    assert r.status_code == 200
