...
def test_secrets_are_write_only(client, make_org):
    ...
    listed = client.get(f"/api/teams/{team['id']}/secrets").json()
...
def test_profile_crud_binding_and_validate(client, make_org):
    ...
    p = client.post(
        f"/api/teams/{team['id']}/profiles",
        json={"name": "mock", "provider": "mock", "model": "mock-1"},
    ).json()
    assert p["id"].startswith("ap_")

    b = client.put(
        f"/api/teams/{team['id']}/bindings",
        json={"agentNodeId": node, "profileId": p["id"]},
    )
    assert b.status_code == 200
    binds = client.get(f"/api/teams/{team['id']}/bindings").json()
    assert [x["agentNodeId"] for x in binds] == [node]

    v = client.post(
        f"/api/teams/{team['id']}/profiles/{p['id']}/validate"
    ).json()
    assert v["ok"] is True  # mock always validates

    up = client.put(
        f"/api/teams/{team['id']}/profiles/{p['id']}", json={"name": "renamed"}
    ).json()
    assert up["name"] == "renamed"

    assert (
        client.delete(f"/api/teams/{team['id']}/profiles/{p['id']}").status_code == 204
    )
