...
// table: experiment_rubric — (experimentId, version) PK; versions append-only
{
  ...
  "promotion": {                       // §4 — the predicate, itself rubric data
    "minScoredTrials": 8, "minWinRate": 0.6,     // amended 2026-08-16; was 5/0.6 — see §4 note
    "holdout": { "minTasks": 4, "minWinRate": 0.6 }  // amended 2026-08-16; was 3/0.5
  },
...
- **Floors** are constraints, not weights — the mechanism behind *"cheaper with no degradation."* `non-inferior` floors compare the challenger's factor against the named reference (usually `champion`) per trial; a floor breach makes the trial a **loss regardless of composite**. This is deliberate: without floors, a 40-point correctness weight still lets enough cost savings buy a correctness deficit. Efficiency searches want quality as a *constraint* and economics as the objective; the floor mechanism expresses that without a second scoring system.
  ...
  > **Amendment (2026-08-16) — anchor floors, against the ratchet.** A floor whose reference is `champion` re-anchors at every promotion: A→B1→B4→B9, each step individually non-inferior within its margin, can walk un-floored quality downhill across generations — nothing in the chain ever compares B9 to A. `floor.reference` therefore gains **`anchor`**: the experiment's designated anchor variant (default: the seed champion at creation; re-anchoring is an explicit operator act, recorded). **Efficiency-search default: correctness floors reference `anchor`, not `champion`**, and rubrics whose promotion predicate leans on judged quality factors should give the load-bearing one an anchor floor with a stated margin. The lineage view marks the anchor so drift-vs-anchor is a rendered number, not an archaeology project.
...
- **Promotion predicate** (rubric data, §3): win rate ≥ `minWinRate` over ≥ `minScoredTrials` scored rotation trials, all floors held, then confirmation on the holdout set (`01` §5). Only then does `champion-suggested` fire — and promotion itself remains a governed action (`05` §3).
  ...
  > **Amendment (2026-08-16) — defaults tightened.** The original defaults (5 trials at 0.6 = 3-of-5; holdout 3 at 0.5 = 2-of-3) let judge noise fire the predicate far too often, and every false `champion-suggested` card spends the scarcest currency the org has — the operator's ratification attention — while peeking (LAB-D2) compounds the odds. New defaults: **8 rotation trials at ≥0.6 (5-of-8) and holdout 4 at ≥0.6 (3-of-4)**, with `holdoutPct` sized so a promotion check never drains the pool (`01` §5's replacement nag stands). Raising n is the cheap rigor available before sequential methods earn their place (open question 1). Where correctness is **judged rather than probed**, one further gate: at least one of the challenger's winning trials must carry a human audit verdict before `champion-suggested` fires — probe-rich domains get mechanical confidence, judged-only domains get a human spot-weld. Operators running cheap, low-stakes experiments can lower any of this per rubric; the *default* is the safe shape.
...
4. **Calibration probes.** The harness periodically seeds probe trials whose exhibits have a known better side (an accepted deliverable vs. an operator-marked degraded copy). Judges never know which trials are probes. Panel accuracy on probes — *"agrees with ground truth 87% · n 23"* — is displayed on the rubric page, and falling accuracy is the signal to change judge instructions, panel size, or judge model tier. The knob-chip closed loop, applied to judgment itself. *Honest limit (2026-08-16): degraded-copy probes measure gross discrimination; the margins that decide close trials sit below their resolution. Treat probe accuracy as a floor on judge health, not a certificate of fine judgment — the human audit stream (§5.5) is the only calibration that reaches the margins.*

   > **Amendment (2026-08-16) — decorrelate the panel (resolves open question 3).** Three evaluators on the same base model with the same instructions are one bias sampled three times, not three votes — majority aggregation then launders a correlated error into false confidence. Default panel from L3: **lens-specialized** — `correctness-lens`, `maintainability-lens`, `adherence-lens` as distinct judge instruction sets — and, where the gateway offers more than one judge-capable tier or provider, **at most two panel seats share a model**. Panel composition is rubric data (`judging.panelSpec`), displayed beside the accuracy strip; probe accuracy per seat, not just per panel, so a weak lens is visible individually.
...
1. **Statistical rigor** (debt LAB-D2, `06` §6). Win rates with minimum n are honest but crude; sequential testing or Elo-style ratings may earn their place once experiments run hundreds of trials. Deliberately not v1. *(Partially mitigated 2026-08-16: tightened predicate defaults, §4.)*
...
3. ~~**Panel diversity.**~~ **Resolved by amendment (2026-08-16, §5.4):** lens-specialized panels are the L3 default, with a two-seats-per-model cap where provider breadth allows.
