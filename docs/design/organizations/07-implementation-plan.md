...
**Landed (2026-08-09 → 2026-08-15).** C1 [#30–#32], C2 [#33], C3 [#35], C4 [#36/#37] merged to `main`; C5 (`c5-economics`), C6 (`c6-provider-breadth`), C7 (`c7-hardening`) built as a stacked branch train awaiting merge. Where each is proven: C1 `test_c1_portfolio_demo.py` + `test_migrate_v1.py`; C2–C3 `test_capacity.py`; C4 `test_scheduler.py`; C5 `test_economics.py` (SC-5 done-bar); C6 `test_provider_breadth.py`; C7 `test_c7_hardening.py` (restart-mid-hold audit, redelivery idempotency, sweep isolation, reading retention, cadence-consults-governor) + the marked live smoke `test_live_smoke.py` (`-m live`, ran green against the operator's Max login 2026-08-15). Deviations and open items from the plan as written are registered in `../../actuation/phase3-debts.md` (C-series close-out: CAP-D6 gateway header wiring, CAP-D7 portfolio SSE, CAP-D8 UI editors for chain/extra-usage, CAP-D9 per-org reserve depth, CAP-D10 cadence runway refinement + trigger governance, CAP-D11 the C5 Playwright arc); §7's doc-edit impact map was executed at adoption (PR #30) with the C7 residue folded in (root README quickstart + status, `02` §9.3, `04` §9.4, `testing.md` §6).
