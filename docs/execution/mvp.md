...
**Sequencing gate (adopted 2026-08-09):** the full Canopy MVP is **not declared delivered until the C-series lands** — the Team/Organization rename and capacity governance of `../design/organizations/07-implementation-plan.md` §0 (operator decision, 2026-08-08). The C1 rename only gets more expensive with accumulated state, and no standing-intent org may run unattended before capacity governance exists. *Gate status (2026-08-15): C1–C7 built (`07` §1 "Landed"); standing intents — cadences — now consult the governor (`04` §9.4). Declaring delivery is the operator's release call; the release checklist's live path is `testing.md` §6.*
...
```
   ...
   a_be  (backend-engineer)   a_qa  (qa-engineer)     cli-claude · ICs
             ◀── dependency: a_qa depends on a_be (verify — resolves at delivery) ──
```
...
4. **Plan review** (X3/U-3): the lead's *staged* fan-out appears — the actual proposed delegations, as unfunded drafts: implement→`a_be` (contract `PullRequest`, 200k), verify→`a_qa` (contract `TestReport`, `dependsOn` the engineer's assignment, `resolveOn: delivered`). Operator approves; the batch funds and dispatches atomically. QA's assignment is born `gated(dependency)` — a padlock on the chart, burning nothing.
5. **Implementation**: the engineer's session plans (visible stages), edits its worktree, runs unit tests, produces the PR artifact. Operator opens the **living plan view** mid-flight — the whole engagement as one outline (lead's plan, both children, cursors, spend) — drops an anchored **note** on the engineer's active stage ("prefer the streaming writer"), which reaches the session at its next turn without suspending anything, then drills into the **inspector** for per-turn Steps with token counts and the meter arc filling.
...
7. **Dependency resolution**: the engineer's `finish` (PR *submitted*, not signed off) auto-resolves QA's verify-dependency — refs pinned at `PullRequest@1` — and wakes the lead for review. The lead holds acceptance; QA runs the suite.
8. **The rework loop** (the formation's whole point): the seeded edge-case test lives in the suite only QA can run, so a first-attempt failure is structural, not model luck. QA's `TestReport` says fail → lead **rejects** the engineer's deliverable — still open in `delivering` — with the report cited, brief unchanged → the assignment returns to `planning` on the *same meter*: rework visibly burns the engineer's budget. Engineer fixes and re-delivers `PullRequest@2`; the lead sends QA a re-verification citing `@2`; green.
9. **Governed merge**: lead **accepts** both deliverables — acceptance is the final verdict, informed by verification — and requests merge → ApprovalGate → operator approves → merge + attestation.
...
| **E2** | Delegation, gates, acceptance | `delegate` + `dependsOn` (dependency gates with per-edge `resolveOn: delivered\|accepted`; resolution hooks at both `finish` and `accept`), **staged delegation** (`proposed` state, batch dispatch on approval — X3 reviews the real fan-out), manager-await wake-on-any-delivery with re-arm, clarification + escalation + approval gates, accept/reject with rework-funding rule, X1 operator intervene, R2 reassign, R3 priority, plan store (+ stage timestamps), anchored notes (`work_note` + turn-boundary injection), trigger sweep (warn/hard-stop/stall), notifications table. Golden state-machine tests (every transition in `work-model.md` §2.1 has a vector, incl. `proposed` and both dependency thresholds). | scripted mock org: 3-node fan-out with a verify-dependency, a rejection funding rework on the same meter while the assignment is still open, a hard-stop resolved by top-up — all via API |
...
| **E5** | Operate UI | Mission control overlay + org pulse, agent inspector (all eight tabs), intent console with plan review (the proposed-batch card) and tree, **living plan view** (outline projection over `GET /intents/{id}/plan`, with inline notes; chart-overlay and timeline projections fast-follow), inbox (needs-you + digest, inline resolutions), cost explorer (by-intent/by-node), SSE channel. | the full §3 scenario driven from the UI, no curl |
...
| **E8** | *Capstone:* the first self-PR | Canopy working on Canopy, one rung early: re-role the three nodes as a docs pod (`tech-writer` / `editor` / lead), point the E4 git executor at a local clone of the Canopy repo, and run **one operator-curated documentation issue** through the full machinery — staged plan review, worktree edit, verify-dep review, acceptance. No new integrations: the operator pastes the issue text as the intent and pushes the accepted branch by hand (no GitHub pack, no ingestion, no auto-PR). Docs-only work carries no execute-class grants, so the subprocess tier stays honest — no waiver stretching. A **live** milestone like the smoke test, never a CI job (CI's fixture remains `target-app`). Seeds `../org-roadmap.md` O2. | a merged PR on the Canopy repo authored by a Canopy org, with its cost receipt — the first entry in the receipts feed |
...
- The rework round provably runs on the *same assignment and meter* (brief unchanged), and acceptance never precedes the green `TestReport` — the §3.7–3.9 ordering is asserted by test, not narrated.
- Nothing dispatches before plan-review approval: `proposed` drafts hold no meter and publish nothing, and the approved batch dispatches atomically.
- A note left mid-flight reaches the target session at its next turn without suspending the assignment.
