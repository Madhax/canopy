...
**The assignment tree's home is actuation-independent (F13/F16).** The CLI keys conversations by project *directory*, so the `assignments/` tree must not live under a path that embeds the actuation id — that made every deactuate → re-actuate orphan the transcript and fail `--resume` with "No conversation found". The actuator passes `CANOPY_WORK_ROOT` (`data/work/<orgId>/<nodeId>`), and the adapter roots `assignments/<id>/` there: the position owns its conversations the same way it owns work, meters, and memory (E6). The same home carries the org-owned audit surface: the node's adapter log (`logs/<nodeId>.log`, size-rotated), each session's stderr (`assignments/<id>/session.stderr.log`, size-rotated), the transcript pointer reported with `session-ref` (stored as `transcript_path`), and an archived copy of each session transcript (`assignments/<id>/transcripts/<sessionId>.jsonl`, copied at session exit) — all readable via the inspector. Absent `CANOPY_WORK_ROOT` the adapter falls back to its cwd (tests, pre-F13 layouts), and the "No conversation found" fresh-session fallback remains as a safety net.
...
**Operator setup: workspace trust (F10c, observed live on Windows).** Headless sessions still honor Claude Code's per-directory workspace trust, stored in the config dir's `.claude.json` under `projects` — one key per project directory, `{"<absolute path>": {"hasTrustDialogAccepted": true, …}}`. The key is the *exact path string*: on Windows, `D:\work\…` and `D:/work/…` are **distinct entries**, and the Canopy sandbox resolves the **forward-slash** form — a trust entry pasted with backslashes silently doesn't match, and sessions stall on the trust prompt they can never answer. When provisioning the dedicated Canopy CLI profile, add the trust entry for the node work root using forward slashes (or run one interactive `claude` from that directory and accept the dialog). F13's stable work home (`data/work/<orgId>/<nodeId>`) makes this a one-time setup per node instead of per actuation.
