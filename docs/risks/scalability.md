...
**Answered (2026-08-09).** The portfolio scheduler is this risk's standing answer: per-team concurrency caps and pacing knobs with predicted effects, org-level capacity shares over each provider pool, reserve watermarks that keep interactive work admissible while batch fleets run hot, and deterministic contention ordering — `design/organizations/04-scheduling-and-throttles.md` §6, with the SC-5 scenario pinned as a scheduler test at milestone C5 (`07` §1).
